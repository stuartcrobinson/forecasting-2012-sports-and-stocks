...
public class GoogleWorldMinuteCruncher_open {

	static int 	[]		delay_ 		= new int [] 	{  1 };			//0 or 1 or 2
	static double []	priceMin_	= new double[]	{ 0.1 }; //0.01, 0.1, 1, 5, 10 };	
	...
	static double []	changeMax_ 	= new double[]	{ -0.8 }; //-0.05, -0.1, -0.15, -0.2, -0.25, -0.3, -0.35, -0.4, -0.5, -0.6, -0.7, -0.8, -0.9, -1	};

	static boolean sell_off_early	= false;
	static boolean displayDetails 	= true;
						...
						for (double changeMin : changeMin_){
							for (double changeMax : changeMax_){
								...
								for (String key : dataMap.keySet()){
									...
									String exchange 		= new String(filename[1]);
									String MARKET_OPEN 		= new String(filename[2]);																//	String INTERVAL 		= new String(filename[4]);		
									String MARKET_CLOSE 	= new String(filename[3]);																//	String TIMEZONE_OFFSET 	= new String(filename[5]);
									...
									for (int r = 0; r < priceListLen - delay - 1; r++){ 
										...
										if (time == 0 && r > 0){
											...
											if (bought){	
												...
												if (sellPrice == -1) 	throw new Exception ("sellPrice is wrong");					//sellPrice = buyPrice *(1.0 - spreadPenalty);
