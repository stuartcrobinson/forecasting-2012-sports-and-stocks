...
*
*in the morning, get the bets and results from the previous day. also get the times of that day's games.
*10 minutes or so before the game, get the top users' bets.  calculate magicbet, and post to wagerline (and wherever else)
*/

class AnalyzeHelper{

	public void printResults(TreeSet<Integer> params, List<Game2> games) {
		String str_ATS = "ATS:";
		String str_OU = "OU: ";
...
class Game2{
	...
	private int calculateUnits(String win) {
		if (win == "1")
			return 500;
		if (win == "0")
			return -550;
	...
	public void calculateWinsAndUnits() throws Exception {
		...
		for (int param : params) {
			...
			Integer magicUnitsATS = calculateUnits(magicWinATS);
			Integer magicUnitsOU = calculateUnits(magicWinOU);
...
//			magicBetOU = testToggleMagicBet(magicBetOU);
	...
	public Game2(int ordinal, TreeSet<Integer> params, String gameID, Date date, String tm1, String tm2, double lineATS, double lineOU, String resultATS, String resultOU, 
	...
	public String toString(){
		String c = ", ";
		return gameID +c+ date +c+ tm1 +c+ tm2 +c+ lineATS +c+ lineOU +c+ resultATS +c+ resultOU +c+ userBetsATS.toString() +c+ userBetsOU.toString();
	}
	...
	public void restictMagicBetsByLineATS(int lineATSMin,	int lineATSMax) {
		...
		if (Math.abs(lineATS) < lineATSMin)
			useThisGame = false;

		if (Math.abs(lineATS) > lineATSMax)
			useThisGame = false;
...
public class Analyze_nonML {
	...
	public static void main(String[] args) throws Exception {
		...
		List<Game2> games = new ArrayList<Game2>();
		String tabulatedDataLocation = "C:\\Users\\User\\Documents\\forecasting\\crowdsourced\\temp_data\\tabulated_NHL_2011_fiftyUsers.csv";
		...
		//gameID,date,tm1,tm2,tm1Pts,tm2Pts,lineATS,lineOU,resultATS,resultOU,betsATS_1,betsOU_1,betsATS_2,betsOU_2,betsATS_10,betsOU_10,
...
//			System.out.println(i);
			...
			String lineATS_s		= line.get(6);
			String lineOU_s			= line.get(7);
			String resultATS		= line.get(8);
			String resultOU			= line.get(9);

			double lineATS, lineOU;

			try { 					lineATS		= Double.parseDouble(lineATS_s);}
			catch (Exception e){	lineATS		= 0;}
			...
			games.add(new Game2(i, unitsParameters, gameID, date, tm1, tm2, lineATS, lineOU, resultATS, resultOU, allBetsATS, allBetsOU));
		}
		System.out.println("\n");
		for (Game2 game : games){
		...
		new AnalyzeHelper().printResults(unitsParameters, games);
		br.close();
	}
